...
final class EquRepairStatsService
{
  ...
  public function getSummary(string $key): array
  {
    ...
    $sql = "
      ...
      JOIN equ_vendors vd ON vd.id = h.vendor_id
      WHERE h.repair_date BETWEEN :s AND :e
      GROUP BY vd.id, vd.name
      ORDER BY `count` DESC, grand_total DESC, vd.name ASC
    ";
    $stmt = $this->db->prepare($sql);
    ...
    foreach ($rows as &$r) {
      $vid = (int)$r['vendor_id'];
      $r['top3'] = $this->getTop3ToolsByVendor($key, $vid);
    }
    unset($r);
  ...
  /** 右側明細：以 headers 為一列（廠商下的事件） */
  public function getDetails(string $key, int $vendorId): array
  {
    [$start, $end] = $this->keyToRange($key);
    ...
    $sql = "
      ...
      WHERE h.vendor_id = :vid
        AND h.repair_date BETWEEN :s AND :e
      GROUP BY
        h.id, h.repair_date, h.repair_type,
        h.company_amount_total, h.team_amount_total, h.grand_total
      ORDER BY h.repair_date DESC, h.id DESC
    ";
    $stmt = $this->db->prepare($sql);
    $stmt->execute([':vid' => $vendorId, ':s' => $start, ':e' => $end]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
  }

  /** Top3：以「工具名稱」統計（同廠商、同期間），count>=3 才顯示，最多3筆 */
  private function getTop3ToolsByVendor(string $key, int $vendorId): array
  {
    [$start, $end] = $this->keyToRange($key);
    ...
    $out = [];
    foreach ($rows as $r) $out[] = (string)$r['tool_name'];
    return $out;
  }
  ...
  public function getPrintVendorMatrix(string $key): array
  {
    ...
    return [
      ...
      'period_text' => $this->keyToPeriodText($key),
      'months' => $months,
      'rows' => $outRows
    ];
  }
