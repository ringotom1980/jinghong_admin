  ...
  var Mod = {
    ...
    openVehAdd: function () {
      ...
      var bd = global.Modal.open({
        ...
        html: ''
          + '<div class="hot-form">'
          + '  <div class="hot-field">'
          + '    <label class="form-label">車輛<span class="hot-req">*</span></label>'
    ...
    openVehDelete: function (vehicleId) {
      ...
      if (self.app && self.app.state && Array.isArray(self.app.state.vehicles)) {
        ...
        if (v) { meta = vehicleLabel(v); cnt = Number(v.assigned_cnt || 0); }
      }
      ...
      apiGet('/api/hot/assign', { action: 'tools', vehicle_id: vehicleId }).then(function (j) {
        ...
        if (j && j.success) {
          ...
          if (rows.length) {
            ...
            toolsTxt = parts.join('，') + (rows.length > 10 ? ' …(共 ' + rows.length + ' 筆)' : '');
          }
        }
