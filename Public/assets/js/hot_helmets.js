...
(() => {
    ...
    async function apiGet(url) {
        // ✅ 用 api.js 已提供的 apiGet（會自動 join BASE_URL）
        const j = await window.apiGet(url);
        if (!j || !j.success) throw new Error(j?.error?.message || j?.error || 'API 失敗');
        return j.data;
    }

    async function apiPost(action, body) {
        // ✅ 用 api.js 已提供的 apiPost（api.js 會自己 JSON.stringify）
        const j = await window.apiPost('api/hot/helmets', { action, ...body });
        if (!j || !j.success) throw new Error(j?.error?.message || j?.error || 'API 失敗');
        return j.data;
    }
